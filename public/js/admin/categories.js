...
$(document).ready(function (e) {
  ...
  $("body").on("click", '.js-creat-category', function () {
    var form_data = $("#js-creat-category").serialize();
  ...
  $('body').on('focusout', '.js-edit-field-category input[type=text], .js-edit-field-category textarea', function () {
    var field_edit = $(this);
    var category_id = field_edit.parent('td').attr('data-category-id');
  ...
  $("body").on("change", '.js-edit-field-category .js-delete-category', function () {
    ...
    var category_id = field_delete.parent('td').attr('data-category-id');
...
function editCategoryFields(category_id, field_name, value) {
  ...
  $.when($(".js-result-message").animate({
    right: 30
  }, 500)).done(function (x) {
    $.ajax({
      type: "POST",
      ...
      error: function error(result) {
        $('td[data-category-id=' + category_id + '][data-name-field=' + field_name + '] textarea').val(result.responseJSON.service_key); // $(".js-result-message").addClass("js-error-message");
        // setTimeout(function(){ $.when( $(".js-result-message").animate({right: -100}, 500) ); }, 1000);
